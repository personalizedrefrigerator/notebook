define([
  ...
  var snr = function(env, event) {
    ...
    var button_labels = [i18n.msg._("Cancel")];

    mod = dialog.modal({
      show: false,
      title: i18n.msg._("Find"),
      body:form,
  ...
  var load = function(keyboard_manager){
    var action_all = {
        cmd: i18n.msg._('find'),
        help: i18n.msg._('find'),
        handler: function(env, event){
          snr(env, event);
        }
    };

    var act_all = keyboard_manager.actions.register(action_all, 'find', 'jupyter-notebook');

    keyboard_manager.command_shortcuts.add_shortcuts({
        'g': 'jupyter-notebook:find'
    });
  };
